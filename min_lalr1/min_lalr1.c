/*
	...
	This software is detached from the Phorward Foundation Libraries, and is
	compiled on its own using the standard C library.
*/
...
void print_c_code( void )
{
	...
	NL
	NL
	P( "int get_act_entry( int state, int sym, int* act, int* idx )" )
	P( "{" )
	P( "	int i;" )
	...
	NL
	P( "int get_goto_entry( int state, int sym, int* act, int* idx )" )
	P( "{" )
	P( "	int i;" )
	...
	NL
	P( "int get_keyword( char* s, int* length )" )
	P( "{" )
	P( "	int match = -1;" )
	P( "	*length = 0;" )
	NL
	...
	NL
	P( "int get_symbol( char* s, int* length )" )
	P( "{" )
	P( "	*length = 1;" )
	P( "	return char_map[ *s ];" )
	P( "}" )
	...
	NL
	P( "	sym = get_symbol( pstr, &len );" )
	P( "	kw_sym = get_keyword( pstr, &kw_len );" )
	NL
	P( "	do" )
	P( "	{" )
	P( "		ok = 0;" )
	...
	P( "		if( kw_sym > -1 )" )
	P( "		{" )
	P( "			ok = get_act_entry( stack[ tos ], kw_sym, &act, &idx );" )
	P( "			length = kw_len;" )
	P( "		}" )
	NL
	P( "		if( !ok )" )
	P( "		{" )
	P( "			ok = get_act_entry( stack[ tos ], sym, &act, &idx );" )
	P( "			length = len;" )
	P( "		}" )
	NL
	P( "		#if PDEBUG" );
	...
	P( "				if( ( act = get_act_entry( stack[ tos ], ERRTOKEN, act, &err ) ) != ERROR )" )
	P( "				{" )
	P( "					tos++;" )
	P( "					stack[ tos ] = act;" )
	...
	P( "						if( kw_sym > -1 )" )
	P( "						{" )
	P( "							act = get_act_entry( stack[ tos ], kw_sym );" )
	P( "							length = kw_len;" )
	P( "						}" )
	NL
	P( "						if( act == ERROR )" )
	P( "						{" )
	P( "							act = get_act_entry( stack[ tos ], sym );" )
	P( "							length = len;" )
	P( "						}" )
	NL
	...
	*/
	...
	P( "			tos -= pop_tab[ idx ][0];" )
	P( "			tos++;" )
	P( "			get_goto_entry( stack[ tos - 1 ], pop_tab[ idx ][1], &act, &idx );" )
	P( "			stack[ tos ] = idx;" )
